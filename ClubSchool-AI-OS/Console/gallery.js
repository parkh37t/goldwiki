...
const GROUPS = [
  {
    ...
    tag: '제안 패키지',
    items: [
      { icon: '📊', title: '경영 요약 (프리미엄 디자인)', desc: 'A4 3페이지 · 윈테마·배점·KPI·Bid 권고. 클라이언트 제출용.', kind: 'design', path: 'Examples/Meritz_Executive_Brief.html' },
      { icon: '📋', title: 'RFP 분석 패키지', desc: '요구·평가매핑·리스크·WBS·Bid 권고(자체 88점).', kind: 'md', path: 'Examples/RFP_Analysis_Meritz_2025.md' },
      { icon: '📄', title: '제안서 본문', desc: 'RFP 필수 목차·평가 80:20 대응·조직 운영방안.', kind: 'md', path: 'Examples/Meritz_제안서_본문.md' },
      { icon: '🩺', title: '별첨4 ① 보장분석 SB', desc: '모바일 보장분석 전환율 개선 화면설계.', kind: 'md', path: 'Examples/Meritz_별첨4_보장분석_SB초안.md' },
      { icon: '🐶', title: '별첨4 ② 펫보험 SB', desc: '다이렉트 펫보험 가입 이탈율 개선 화면설계.', kind: 'md', path: 'Examples/Meritz_별첨4_펫보험_SB초안.md' },
      { icon: '📱', title: '보장분석 동작 프로토타입', desc: '5화면 클릭 시연 (모바일).', kind: 'proto', path: 'Examples/prototypes/meritz-bojang-analysis.html' },
      { icon: '📱', title: '펫보험 동작 프로토타입', desc: '6화면 클릭 시연 (모바일).', kind: 'proto', path: 'Examples/prototypes/meritz-pet-insurance.html' },
    ],
  },
  {
    ...
    tag: '레퍼런스',
    items: [
      { icon: '🎨', title: '제안 경영요약 (1p 디자인)', desc: '프리미엄 1페이지 디자인 산출물.', kind: 'design', path: 'Examples/design-onepager.html' },
      { icon: '📋', title: 'RFP 심층 분석 보고서', desc: '요구·평가·리스크 분석.', kind: 'md', path: 'Examples/01_RFP_Analysis.md' },
      { icon: '📄', title: '제안 경영 요약', desc: 'Executive Summary.', kind: 'md', path: 'Examples/02_Proposal_Executive_Summary.md' },
    ],
  },
  {
    name: '지식베이스',
    tag: 'GoldWiki',
    items: [
      { icon: '📚', title: 'GoldWiki 통합본', desc: '전체 지식 105개 문서 통합 (NotebookLM 소스용).', kind: 'md', path: 'Examples/GoldWiki_Combined.md' },
    ],
  },
];
...
function cardHtml(g, i, it) {
  ...
  return `<div class="g-card">
    <div class="g-card-top"><span class="g-ico">${it.icon}</span><span class="g-kind">${badge(it.kind)}</span></div>
    <h3>${esc(it.title)}</h3>
    <p>${esc(it.desc)}</p>
    <div class="g-card-act">
      <button class="btn primary" data-open='${data}'>${primaryLabel(it.kind)}</button>
    </div>
  </div>`;
}
